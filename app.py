...
with tab1:
    ...
    # Tighter strike filter (default now 3%)
    st.sidebar.header("Filters")
    range_percent = st.sidebar.slider("Show strikes within ± % of spot", 1, 30, 3)   # ← Changed default to 3%
...
with tab2:
    ...
    try:
        ...
        if not df.empty:
            ...
            fig.add_hline(y=spot, line_dash="dash", line_color="#ffd700", annotation_text=f"SPOT = {spot:,.0f} ← MAX MAGNET", annotation_position="top right")
...
st.caption("✅ GEX Dashboard with tighter 3% strike filter + GEX Profile tab")
